...
class ZNM2Net(nn.Module):
    ...
    def forward(self, x):
        ...
        if x.size(0) > 1:
            x = self.drop1(self.relu4(self.bn4(self.fc1(x))))
        else:
...
def train_model(dataset_path, target_acc, save_path):
    try:
        ...
        dataset = CKPlusDataset(dataset_path)
        ...
        dummy_input = torch.randn(1, 1, 48, 48, dtype=torch.float32)
